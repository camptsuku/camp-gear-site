...
const EXCLUDE_WORDS = [
  ...
  '運動会', '学校', '業務用', 'パーティー', 'マーケット', 'EZ UP', 'EZUP',
];
...
function matchesCategory(name, category) {
  ...
  if (!keywords) {
    // 未定義カテゴリ: 既知カテゴリのどれかにマッチすれば通す（誤カテゴリ名対策）
    // どれにもマッチしなければ弾く
    return Object.values(CATEGORY_KEYWORDS).some(kws => kws.some(kw => name.includes(kw)));
  }
  if (category === 'テント') {
    const tentExcludes = ['イージーアップ', 'タープテント', 'ワンタッチタープ', 'イベント', '運動会'];
    if (tentExcludes.some(w => name.includes(w))) return false;
  }
...
【重要制約】純粋なキャンプ・アウトドア用ギアのみ提案すること。家庭用調理器具・家電・食品・衣類・日用品・インドア用品・防災用品・医療用品・介護用品は絶対に含めないこと。クッカーはアウトドア用クッカー・バーナー・メスティンのみ（家庭用鍋・フライパン・炊飯器・低温調理器・ゆで卵メーカー等は禁止）。本体製品のみ（アクセサリー・パーツ・収納ケース除く）。JSONのみ:
